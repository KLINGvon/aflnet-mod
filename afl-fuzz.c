    ...
    if (state_count > 1) {
      ...
      for(i=1; i < state_count; i++) {
        ...
        Agnode_t *from, *to;
          ...
          // ++ 新增代码 ++
          newState_To->discovered_at_cycle = queue_cycle ? queue_cycle : 1;
...
/*
  Helper function to rescan the buffer and rebuild message boundaries.
  This is crucial after any mutation that alters buffer length or structure.
*/
static void rescan_and_update_boundaries(u8* buf, u32 len, u32** boundaries_ptr, u32* count_ptr) {
  
  // 1. Free the old boundaries array if it exists.
  if (*boundaries_ptr) {
    ck_free(*boundaries_ptr);
    *boundaries_ptr = NULL;
  }

  // 2. Use the protocol-specific function to re-identify regions/messages.
  region_t* regions = (*extract_requests)(buf, len, count_ptr);

  // If no regions are found, we can't create boundaries.
  if (!regions || *count_ptr == 0) {
    if(regions) ck_free(regions);
    // You might want to handle this case, perhaps by setting count to 0 and returning.
    *count_ptr = 0;
    return;
  }
  
  // 3. Allocate memory for the new boundaries array.
  *boundaries_ptr = ck_alloc(sizeof(u32) * (*count_ptr + 1));
  (*boundaries_ptr)[0] = 0; // The first boundary is always at the start.

  // 4. Populate the new boundaries array based on the new regions.
  u32 i;
  u32 current_offset = 0;
  for (i = 0; i < *count_ptr; i++) {
    u32 region_len = (regions[i].end_byte - regions[i].start_byte + 1);
    current_offset += region_len;
    (*boundaries_ptr)[i + 1] = current_offset;
  }

  // 5. Free the temporary regions array.
  ck_free(regions);
}
...
havoc_stage:
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    for (i = 0; i < use_stacking; i++) {
      ...
      // int structural_mutation = 0; // A flag to track if we need to rescan
